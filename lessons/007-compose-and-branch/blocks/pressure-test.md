...
There is no state in which this loop decides it is finished and no verdict that ends it. It stops
when
a human stops pressing Enter, and the human pressing Enter is doing the one job the orchestrator was
supposed to have taken over — the lexicon lists "deciding when the line is finished" alongside
routing,
