...
Tutor, Practice Coach, Judge, Docker, visual, or live-eval calls to it.
...
unset PRACTICE_COACH_LOG_PROMPT
...
5. Authenticate Pi on the host for the Main Tutor, Practice Coach, and Judge providers.
6. Set explicit models, either by flags where supported or by environment:
   `TUTOR_MODEL`, `PRACTICE_COACH_MODEL`, and `EVAL_JUDGE_MODEL`.
7. Set `EVAL_JUDGE_COMMAND` explicitly, for example `pi --no-session` or a Pi-compatible wrapper.
8. Leave `PRACTICE_COACH_LOG_PROMPT` unset or disabled. Live evals refuse private prompt logging.

Paid calls can occur in three roles: Main Tutor, Practice Coach, and Judge. Deterministic gates do
...
All authored live scenarios still use the real Main Tutor and Practice Coach review paths. Post-L001
scenarios use driver-private command stubs only for selected authored learner shell commands. The
...
fixture validation, and privacy checks happen before Tutor, Practice Coach, or Judge probes can
spend tokens.
