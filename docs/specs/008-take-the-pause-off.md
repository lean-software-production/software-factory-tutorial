...
responsible for "starting a line, handing each machine its inputs, choosing what runs next where the
...
Every machine so far has been handed its inputs and run to completion with no memory of the turn
...
in the commits. Five iterations, four machines each, and the number does not exist.
