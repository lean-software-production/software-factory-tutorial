...
export type AuthoredWorkbookEvalRoleKey = "mainTutor" | "practiceCoach" | "judge";
export type AuthoredWorkbookEvalRoleLabel = "Main Tutor" | "Practice Coach" | "Judge";
...
export interface AuthoredWorkbookEvalRoleDefinition {
  ...
  env: typeof TUTOR_MODEL_ENV | typeof PRACTICE_COACH_MODEL_ENV | typeof EVAL_JUDGE_MODEL_ENV;
}

export const WORKBOOK_EVAL_ROLES = Object.freeze([
  Object.freeze({ key: "mainTutor", label: "Main Tutor", env: TUTOR_MODEL_ENV }),
  Object.freeze({ key: "practiceCoach", label: "Practice Coach", env: PRACTICE_COACH_MODEL_ENV }),
...
export interface AuthoredWorkbookEvalExternalOperations {
  ...
  probePracticeCoach(input: AuthoredWorkbookEvalPaidRoleInput): Promise<AuthoredWorkbookEvalPaidRoleResult>;
...
const PUBLIC_FAILURE_MESSAGES: Readonly<Record<AuthoredWorkbookEvalPreflightPhase, string>> = Object.freeze({
  ...
  promptLogging: "Forbidden private prompt logging is enabled.",
  ...
  practiceCoach: "Practice Coach paid preflight failed.",
...
export function formatAuthoredWorkbookEvalPreflightHelp(): string {
  return [
    ...
    "  --practice-coach-model <provider/model>    Also accepted from PRACTICE_COACH_MODEL.",
...
function validateAuthoredWorkbookEvalPreflightRequestInternal(input: AuthoredWorkbookEvalPreflightRequestInput, catalog: readonly (AuthoredWorkbookEvalScenario | AuthoredWorkbookScenarioDescriptor)[]): AuthoredWorkbookEvalPreflightRequest {
  ...
  assertPrivatePromptLoggingDisabled(environment);
...
async function runPaidRole(
  operation: "mainTutor" | "practiceCoach",
...
export function defaultPreflightOperations(dependencies: AuthoredWorkbookEvalDefaultOperationDependencies = {}): AuthoredWorkbookEvalExternalOperations {
  ...
  return {
    ...
    probePracticeCoach: createDefaultWorkbookRoleProbe("Practice Coach"),
...
export function createDefaultWorkbookRoleProbe(role: "Main Tutor" | "Practice Coach"): (input: AuthoredWorkbookEvalPaidRoleInput) => Promise<AuthoredWorkbookEvalPaidRoleResult> {
  return async ({ request, timeoutMs, signal }) => {
    const envVar = role === "Main Tutor" ? TUTOR_MODEL_ENV : PRACTICE_COACH_MODEL_ENV;
...
export function snapshotAuthoredWorkbookEvalEnvironment(environment: NodeJS.ProcessEnv, modelOverrides: Partial<Record<AuthoredWorkbookEvalRoleKey, string>> = {}): AuthoredWorkbookEvalEnvironment {
  const keys = new Set<string>([
    ...
    OPENCODE_API_KEY_ENV, TUTOR_MODEL_ENV, PRACTICE_COACH_MODEL_ENV, EVAL_JUDGE_MODEL_ENV, EVAL_JUDGE_COMMAND_ENV, PRACTICE_COACH_LOG_PROMPT_ENV
  ]);
  ...
  if (modelOverrides.practiceCoach !== undefined) snapshot[PRACTICE_COACH_MODEL_ENV] = modelOverrides.practiceCoach;
  if (modelOverrides.judge !== undefined) snapshot[EVAL_JUDGE_MODEL_ENV] = modelOverrides.judge;
  return deepFreezePlain(snapshot);
}

export function assertPrivatePromptLoggingDisabled(environment: AuthoredWorkbookEvalEnvironment): void {
  if (isEnabled(environment[PRACTICE_COACH_LOG_PROMPT_ENV])) throw new AuthoredWorkbookEvalPreflightError("promptLogging", { code: "private_prompt_logging_enabled" });
}
  ...
  return raw.map((scenario) => {
    ...
    return Object.freeze({
      ...
      expectedModelCalls: deepFreezePlain({
        ...
        practiceCoach: expectedModelCalls.byKey.practiceCoach,
...
function modelIdentityFromEnvironment(request: AuthoredWorkbookEvalPreflightRequest, envVar: typeof TUTOR_MODEL_ENV | typeof PRACTICE_COACH_MODEL_ENV): WorkbookModelIdentity {
  const role = envVar === TUTOR_MODEL_ENV ? "mainTutor" : "practiceCoach";
  return { provider: request.models[role].provider, id: request.models[role].id };
}
...
function normalizeExpectedModelCalls(raw: AuthoredWorkbookEvalScenario["expectedModelCalls"]): { byKey: Record<AuthoredWorkbookEvalRoleKey, number>; byRole: Record<AuthoredWorkbookEvalRoleLabel, number>; total: number } {
  ...
  const allowedKeys = new Set<string>(["mainTutor", "practiceCoach", "judge", "total"]);
  for (const key of Object.keys(raw)) if (!allowedKeys.has(key)) throw validationError();
  const byRole = emptyRoleCounts();
  const byKey: Record<AuthoredWorkbookEvalRoleKey, number> = { mainTutor: 0, practiceCoach: 0, judge: 0 };
  let roleSum = 0;
  for (const role of WORKBOOK_EVAL_ROLES) {
...
function emptyRoleCounts(): Record<AuthoredWorkbookEvalRoleLabel, number> {
  return { "Main Tutor": 0, "Practice Coach": 0, Judge: 0 };
}
