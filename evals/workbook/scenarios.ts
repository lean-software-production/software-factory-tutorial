...
function gateLessons003004EvidenceFeedback(input: AuthoredWorkbookScenarioGateInput): AuthoredWorkbookScenarioGateResult {
  ...
  return evaluateGate([
    ...
    assertion("lessons003004-wrong-rerun-before-feedback", terminalInputTexts(input.trace).some((text) => text.endsWith(lesson004WrongCommand)) && tutorFeedbackAfterInput(input.trace, /TURN RESULT: validator reran without appending findings/, /rerun|validator|baseline|findings|append|doer context/i), "The wrong validator-only rerun occurs before feedback and does not re-record the baseline."),
