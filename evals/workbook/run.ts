...
export function createAuthoredWorkbookRunDependencies(primitives: AuthoredWorkbookRunPrimitives = defaultAuthoredWorkbookRunPrimitives): AuthoredWorkbookRunDependencies {
  return {
    ...
    startServer: async ({ workspace, request, summary, signal }) => {
      ...
      const practiceCoach = models.createPracticeCoach({ workspace: workspace.repositoryRoot });
      try {
        throwIfAborted(signal);
...
function modelIdentitiesFromPreflight(request: AuthoredWorkbookEvalPreflightRequest, summary: AuthoredWorkbookEvalPublicSummary): AuthoredWorkbookEvalModelIdentities {
  ...
  return {
    ...
    "Practice Coach": { requested: requested.get("Practice Coach") ?? request.models.practiceCoach.identity, selected: selected.get("Practice Coach") ?? request.models.practiceCoach.identity },
...
function placeholderModelIdentities(): AuthoredWorkbookEvalModelIdentities {
  return { "Main Tutor": { requested: "placeholder/model", selected: "placeholder/model" }, "Practice Coach": { requested: "placeholder/model", selected: "placeholder/model" }, Judge: { requested: "placeholder/model", selected: "placeholder/model" } };
}
