...
import { authoredWorkbookScenarioById } from "../scenarios.js";
...
function deterministicScenario(): AuthoredWorkbookEvalScenarioPublicDescriptor {
  const descriptor = authoredWorkbookScenarioById("lessons-003-004-evidence-feedback");
  return {
    id: descriptor.id,
    title: descriptor.title,
    description: descriptor.description,
    criteria: descriptor.criteria.map((criterion) => ({ id: criterion.id, title: criterion.title, description: criterion.description }))
  };
}

function authoredScenario(): AuthoredWorkbookEvalScenarioPublicDescriptor {
  const descriptor = authoredWorkbookScenarioById("lesson-001-headless-boundary");
  return {
    id: descriptor.id,
    title: descriptor.title,
    description: descriptor.description,
...
function deterministicGate(assertionCount = 17, failedIndex?: number): AuthoredWorkbookEvalGateResult {
  return { passed: true, assertions: Array.from({ length: assertionCount }, (_, index) => ({ name: `deterministic-private-${index}`, passed: failedIndex === index ? false : true, detail: `private deterministic detail ${index} /tmp/private-gate-path` })) };
}
...
function authoredJudgeResult(descriptor: AuthoredWorkbookEvalScenarioPublicDescriptor = authoredScenario()): AuthoredWorkbookEvalJudgeResult {
  return {
    criteria: Object.fromEntries(descriptor.criteria.map((criterion) => [criterion.id, { score: 2, citations: [0], rationale: `Public trace evidence supports ${criterion.title}.` }])) as AuthoredWorkbookEvalJudgeResult["criteria"],
    summary: "The public trace supports every authored catalog criterion."
  };
}
...
function replacePromptTrace(prompt: string, originalTrace: unknown, replacementTrace: unknown): string {
  const original = JSON.stringify(originalTrace, null, 2);
...
async function authoredJudgedLatestFixture(root: string, runId = "run-authored-001") {
  ...
  const judge = authoredJudgeResult(scenario);
  ...
  const latest = createAuthoredWorkbookEvalLatestEnvelope({
    generatedAt: "2026-08-29T00:00:00.000Z",
    invocation: { scope: "scenario", scenarioIds: [scenario.id], repeat: 1 },
    runs: [createAuthoredWorkbookEvalLatestRunEntry({
      scenario: scenario.id,
      repetition: 1,
      status: "completed",
      evaluationMode: "judged",
      verdict: { passed: true, percentage: 1, rule: "all-criteria-positive-and-aggregate-at-least-80-percent" },
      reportDirectory: runId,
      files: AUTHORED_WORKBOOK_REPORT_FILENAMES
    })]
  });
  await writeAuthoredWorkbookEvalLatestEnvelope(root, latest);
  const latestText = await readFile(join(root, "latest.json"), "utf8");
  return { scenario, trace, judge, prompt, written, latest, latestText };
}

async function expectLatestRejectedAndUnchanged(root: string, latest: ReturnType<typeof createAuthoredWorkbookEvalLatestEnvelope>, prior: string): Promise<void> {
  await expect(writeAuthoredWorkbookEvalLatestEnvelope(root, latest)).rejects.toThrow("Unable to update authored workbook latest report.");
...
describe("authored workbook report bundle", () => {
  it("re-verifies prompt and judge, writes exact curated files, and keeps diagnostics separate", async () => {
    ...
    const trace = authoredTrace();
    const prompt = buildAuthoredWorkbookJudgePrompt(authored, trace, gate());
    const result = await writeAuthoredWorkbookEvalReportBundle({
      reportsRoot: root,
      runId,
      scenario: { ...authored, tutorFrontmatter: "frontmatter-secret" } as unknown as AuthoredWorkbookEvalScenarioPublicDescriptor,
    ...
    expect(report.files).toEqual(AUTHORED_WORKBOOK_REPORT_FILENAMES);
    expect(report).not.toHaveProperty("trace");
    expect(report).not.toHaveProperty("judgeInput");
    expect(report).toMatchObject({ evaluationMode: "judged", judge: { policy: "scenario-specific", expectedCalls: 1, status: "completed" } });
    expect(metadata).toMatchObject({ evaluationMode: "judged", files: AUTHORED_WORKBOOK_REPORT_FILENAMES, status: "completed", outcome: "passed", modelIdentities: modelIdentities() });
  ...
  it("writes deterministic-only success without Judge prompt/result artifacts or probabilistic verdict", async () => {
    ...
    expect(written.files).toEqual(AUTHORED_WORKBOOK_DETERMINISTIC_REPORT_FILENAMES);
    await expect(pathExists(join(written.directory, "trace.json"))).resolves.toBe(true);
    await expect(pathExists(join(written.directory, "report.json"))).resolves.toBe(true);
    await expect(pathExists(join(written.directory, "summary.md"))).resolves.toBe(true);
    await expect(pathExists(join(written.directory, "metadata.json"))).resolves.toBe(true);
    ...
    const report = await readJson<Record<string, unknown>>(join(written.directory, "report.json"));
    const metadata = await readJson<Record<string, unknown>>(join(written.directory, "metadata.json"));
    const summary = await readFile(join(written.directory, "summary.md"), "utf8");

    expect(report).toMatchObject({
      runId,
      evaluationMode: "deterministic-only",
      scenario: { id: deterministicScenario().id },
      judge: { policy: "deterministic-only", expectedCalls: 0, status: "not-run" },
      verdict: { passed: true, rule: "deterministic-gate-only" },
      files: AUTHORED_WORKBOOK_DETERMINISTIC_REPORT_FILENAMES
    });
    expect(report).not.toHaveProperty("judgeInput");
    expect(metadata).toMatchObject({
      ...
      outcome: "passed",
      verdict: { passed: true, rule: "deterministic-gate-only" },
      lifecycle: { judge: "not-applicable", completed: "yes", cleanup: "completed" },
      files: AUTHORED_WORKBOOK_DETERMINISTIC_REPORT_FILENAMES,
      modelIdentities: { "Main Tutor": modelIdentities()["Main Tutor"] }
    });
    expect(metadata).not.toHaveProperty("modelIdentities.Judge");
    expect(JSON.stringify(metadata)).not.toContain("gemini");
    expect(summary).toContain("Evaluation mode: **deterministic-only**");
    expect(summary).toContain("Judge: **not run**");
    expect(summary).not.toContain("Judge summary");
    expect(summary).not.toContain("100%");
    expectNoPrivate({ report, metadata, summary });

    const deterministicLatestEntry = createAuthoredWorkbookEvalLatestRunEntry({
      scenario: deterministicScenario().id,
      repetition: 1,
      status: "completed",
      evaluationMode: "deterministic-only",
    ...
    const deterministicLatest = createAuthoredWorkbookEvalLatestEnvelope({ generatedAt: "2026-08-29T00:00:00.000Z", invocation: { scope: "scenario", scenarioIds: [deterministicScenario().id], repeat: 1 }, runs: [deterministicLatestEntry] });
    await writeAuthoredWorkbookEvalLatestEnvelope(root, deterministicLatest);
    const latest = await readJson<Record<string, any>>(join(root, "latest.json"));
    expect(latest.runs).toEqual([expect.objectContaining({ evaluationMode: "deterministic-only", verdict: { passed: true, rule: "deterministic-gate-only" }, files: AUTHORED_WORKBOOK_DETERMINISTIC_REPORT_FILENAMES })]);
    expect(JSON.stringify(latest)).not.toContain("judge-input.json");
    expect(JSON.stringify(latest)).not.toContain("judge.json");
    expect(() => createAuthoredWorkbookEvalLatestRunEntry({ ...deterministicLatestEntry, files: AUTHORED_WORKBOOK_REPORT_FILENAMES })).toThrow(/curated files/i);
    expect(() => createAuthoredWorkbookEvalLatestRunEntry({ ...deterministicLatestEntry, verdict: { passed: true, percentage: 1, rule: "deterministic-gate-only" } as any })).toThrow(/verdict/i);
    expect(() => createAuthoredWorkbookEvalReportBundleObjects({ runId: "bad-deterministic-count", scenario: deterministicScenario(), trace, gate: deterministicGate(16), evaluationMode: "deterministic-only", modelIdentities: { "Main Tutor": modelIdentities()["Main Tutor"] } })).toThrow(/17-assertion gate/i);
    expect(() => createAuthoredWorkbookEvalReportBundleObjects({ runId: "bad-deterministic-failure", scenario: deterministicScenario(), trace, gate: deterministicGate(17, 3), evaluationMode: "deterministic-only", modelIdentities: { "Main Tutor": modelIdentities()["Main Tutor"] } })).toThrow(/failed assertions/i);
    expect(() => createAuthoredWorkbookEvalReportBundleObjects({ runId: "bad-deterministic-scenario", scenario: scenario(), trace: projectedTrace(), gate: deterministicGate(), evaluationMode: "deterministic-only", modelIdentities: { "Main Tutor": modelIdentities()["Main Tutor"] } })).toThrow(/deterministic-only scenario/i);
    expect(() => createAuthoredWorkbookEvalReportBundleObjects({ runId: "bad-judged-deterministic-scenario", scenario: deterministicScenario(), trace, gate: deterministicGate(), evaluationMode: "judged", judgeInput: "fabricated", judge: judgeResult(), modelIdentities: modelIdentities() })).toThrow(/judged scenario/i);
    expect(() => createAuthoredWorkbookEvalLatestRunEntry({ ...deterministicLatestEntry, evaluationMode: "judged", files: AUTHORED_WORKBOOK_REPORT_FILENAMES, verdict: { passed: true, percentage: 1, rule: "all-criteria-positive-and-aggregate-at-least-80-percent" } })).toThrow(/judged scenario/i);
    expect(() => createAuthoredWorkbookEvalReportBundleObjects({ runId: "bad-deterministic-model", scenario: deterministicScenario(), trace, gate: deterministicGate(), evaluationMode: "deterministic-only", modelIdentities: modelIdentities() })).toThrow(/model identities/i);

    expect(() => createAuthoredWorkbookEvalReportBundleObjects({
      runId: "bad-deterministic",
      scenario: deterministicScenario(),
      trace,
      gate: deterministicGate(),
      evaluationMode: "deterministic-only",
  ...
  it("rejects forged deterministic latest directory content and unknown report files", async () => {
    ...
    const latest = createAuthoredWorkbookEvalLatestEnvelope({
      generatedAt: "2026-08-29T00:00:00.000Z",
      invocation: { scope: "scenario", scenarioIds: [deterministicScenario().id], repeat: 1 },
      runs: [createAuthoredWorkbookEvalLatestRunEntry({
        scenario: deterministicScenario().id,
        repetition: 1,
        status: "completed",
    ...
    const reportPath = join(written.directory, "report.json");
    const tracePath = join(written.directory, "trace.json");
    const originalReport = await readJson<Record<string, unknown>>(reportPath);
    const originalTraceEnvelope = await readJson<Record<string, unknown>>(tracePath);
    const writeJson = (path: string, value: unknown) => atomicWriteText(path, `${JSON.stringify(value, null, 2)}\n`);
    const expectLatestRejectedAndUnchanged = async (mutate: () => Promise<(() => Promise<void>) | void>): Promise<void> => {
      ...
      try {
        ...
        await cleanup?.();
        await writeJson(reportPath, originalReport);
        await writeJson(tracePath, originalTraceEnvelope);
      }
    };

    await expectLatestRejectedAndUnchanged(async () => writeJson(reportPath, { ...originalReport, judgeInput: "fabricated prompt private-secret", judgeModel: "google/gemini-private", command: "/tmp/private-judge" }));
    await expectLatestRejectedAndUnchanged(async () => writeJson(reportPath, { ...originalReport, modelIdentities: { ...(originalReport.modelIdentities as Record<string, unknown>), Judge: modelIdentities().Judge } }));
    ...
    await expectLatestRejectedAndUnchanged(async () => {
      ...
      return async () => { await rm(extra, { force: true }); };
    });
    await expectLatestRejectedAndUnchanged(async () => {
      const judgeInput = join(written.directory, "judge-input.json");
      const judge = join(written.directory, "judge.json");
      await writeFile(judgeInput, `${JSON.stringify({ ...AUTHORED_WORKBOOK_EVAL_MARKERS, scenario: deterministicScenario().id, traceFile: "trace.json", prompt: "fabricated" })}\n`, { mode: 0o600 });
      await writeFile(judge, `${JSON.stringify({ ...AUTHORED_WORKBOOK_EVAL_MARKERS, judge: judgeResult(), verdict: { passed: true, percentage: 1, rule: "all-criteria-positive-and-aggregate-at-least-80-percent" } })}\n`, { mode: 0o600 });
      return async () => { await rm(judgeInput, { force: true }); await rm(judge, { force: true }); };
    });

    await expect(writeAuthoredWorkbookEvalGateDiagnostic(written.directory, gate(false))).resolves.toBe("written");
    await expect(writeAuthoredWorkbookEvalLatestEnvelope(root, latest)).resolves.toEqual({ file: "latest.json" });
  });

  it("rejects a latest run whose Judge trace forges unknown projected public-state fields", async () => {
    const root = await mkdtemp(join(tmpdir(), "authored-latest-forged-public-state-"));
    tempRoots.push(root);
    const { written, latest, latestText } = await authoredJudgedLatestFixture(root, "run-forged-public-state");
    const tracePath = join(written.directory, "trace.json");
    const judgeInputPath = join(written.directory, "judge-input.json");
    const traceEnvelope = await readJson<any>(tracePath);
    const judgeInput = await readJson<any>(judgeInputPath);
    const forgedTraceEnvelope = structuredClone(traceEnvelope);
    const forgedState = forgedTraceEnvelope.trace.publicStates[0].state;
    forgedState.privateInternals = { token: "sk-secret-token", paths: ["/tmp/private-state-path"] };
    forgedState.adapter.token = "adapter-token-secret";
    forgedState.chapters = [{ id: "chapter", title: "Chapter", lessonNumber: 1, unknownChapterPolicy: "private-policy", lesson: { id: "lesson", title: "Lesson", durationMinutes: 1, outcomes: [], blockCount: 1, blocks: [{ id: "block", title: "Block", token: "block-token-secret" }] } }];
    forgedState.timelineNewRecords = [{ type: "message", id: "timeline", sequence: 1, lessonId, blockId: "visible", role: "assistant", source: "main_tutor", presentation: "chat", text: { bytes: 1 }, token: "timeline-token-secret" }];
    await writeJson(tracePath, forgedTraceEnvelope);
    await writeJson(judgeInputPath, { ...judgeInput, prompt: replacePromptTrace(String(judgeInput.prompt), traceEnvelope.trace, forgedTraceEnvelope.trace) });

    await expectLatestRejectedAndUnchanged(root, latest, latestText);
  });

  it("rejects a latest run whose report descriptor is a self-consistent forgery instead of the authored catalog descriptor", async () => {
    const root = await mkdtemp(join(tmpdir(), "authored-latest-forged-scenario-"));
    tempRoots.push(root);
    const { written, latest, latestText } = await authoredJudgedLatestFixture(root, "run-forged-scenario");
    const traceEnvelope = await readJson<any>(join(written.directory, "trace.json"));
    const reportPath = join(written.directory, "report.json");
    const judgeInputPath = join(written.directory, "judge-input.json");
    const judgePath = join(written.directory, "judge.json");
    const summaryPath = join(written.directory, "summary.md");
    const report = await readJson<any>(reportPath);
    const forgedScenario: AuthoredWorkbookEvalScenarioPublicDescriptor = {
      id: report.scenario.id,
      title: "Forged authored catalog title",
      description: "Forged descriptor, prompt, judge, and summary are internally consistent but not from the authored catalog.",
      criteria: [
        { id: "forged-criterion", title: "Forged criterion", description: "A forged criterion should not be accepted." }
      ]
    };
    const forgedJudge: AuthoredWorkbookEvalJudgeResult = {
      criteria: { "forged-criterion": { score: 2, citations: [0], rationale: "Self-consistent forged rationale." } },
      summary: "Self-consistent forged Judge summary."
    };
    const forgedVerdict = authoredWorkbookJudgeVerdict(forgedJudge);
    await writeJson(reportPath, { ...report, scenario: forgedScenario, verdict: forgedVerdict });
    await writeJson(judgeInputPath, { ...AUTHORED_WORKBOOK_EVAL_MARKERS, scenario: forgedScenario.id, traceFile: "trace.json", prompt: buildAuthoredWorkbookJudgePromptFromProjectedTrace(forgedScenario, traceEnvelope.trace, report.gate) });
    await writeJson(judgePath, { ...AUTHORED_WORKBOOK_EVAL_MARKERS, judge: forgedJudge, verdict: forgedVerdict });
    await atomicWriteText(summaryPath, renderAuthoredWorkbookSummary({ scenario: forgedScenario, gate: report.gate, evaluationMode: "judged", judge: forgedJudge, verdict: forgedVerdict }));

    await expectLatestRejectedAndUnchanged(root, latest, latestText);
  });

  it("rejects a completed judged latest run with a failed deterministic gate even when the Judge passes", async () => {
    const root = await mkdtemp(join(tmpdir(), "authored-latest-forged-gate-"));
    tempRoots.push(root);
    const { scenario, judge, written, latest, latestText } = await authoredJudgedLatestFixture(root, "run-forged-gate");
    const traceEnvelope = await readJson<any>(join(written.directory, "trace.json"));
    const reportPath = join(written.directory, "report.json");
    const judgeInputPath = join(written.directory, "judge-input.json");
    const summaryPath = join(written.directory, "summary.md");
    ...
    const forgedGate = {
      ...
      assertionCount: 2,
      failureCount: 1,
      assertions: [{ index: 0, passed: true }, { index: 1, passed: false }],
      detailPolicy: "assertion-details-omitted-from-public-report" as const
    };
    const passingVerdict = authoredWorkbookJudgeVerdict(judge);
    await writeJson(reportPath, { ...report, gate: forgedGate, verdict: passingVerdict });
    await writeJson(judgeInputPath, { ...AUTHORED_WORKBOOK_EVAL_MARKERS, scenario: scenario.id, traceFile: "trace.json", prompt: buildAuthoredWorkbookJudgePromptFromProjectedTrace(scenario, traceEnvelope.trace, forgedGate) });
    await atomicWriteText(summaryPath, renderAuthoredWorkbookSummary({ scenario, gate: forgedGate, evaluationMode: "judged", judge, verdict: passingVerdict }));
  ...
  it("rejects mismatched injected prompts, invalid judge results, failed gates, bad run ids, and unsafe model identities", () => {
    ...
    expect(() => createAuthoredWorkbookEvalReportBundleObjects({ ...base, modelIdentities: { ...modelIdentities(), command: "private command secret" } as unknown as AuthoredWorkbookEvalModelIdentities })).toThrow(/model identities/i);
  ...
  it("creates sanitized failure metadata without advertising local diagnostics or judge artifacts", async () => {
    ...
    expect(metadataObject).toMatchObject({ ...AUTHORED_WORKBOOK_EVAL_MARKERS, status: "cleanup", verdict: { passed: false, percentage: 0, rule: "not-judged" }, lifecycle: { completed: "no", interrupted: "no", report: "not-written", cleanup: "failed" }, files: { metadata: "metadata.json" } });

    const deterministicReportFailure = createAuthoredWorkbookEvalFailureMetadataEnvelope({
      runId: "deterministic-report-failure",
      scenarioId: deterministicScenario().id,
      status: "report",
      evaluationMode: "deterministic-only",
      modelIdentities: { "Main Tutor": modelIdentities()["Main Tutor"] }
    });
    expect(deterministicReportFailure).toMatchObject({ status: "report", evaluationMode: "deterministic-only", lifecycle: { judge: "not-applicable", report: "failed", cleanup: "completed" }, files: { metadata: "metadata.json" } });

    const deterministicCleanupFailure = createAuthoredWorkbookEvalFailureMetadataEnvelope({
      runId: "deterministic-cleanup-failure",
      scenarioId: deterministicScenario().id,
      status: "cleanup",
      evaluationMode: "deterministic-only",
      modelIdentities: { "Main Tutor": modelIdentities()["Main Tutor"] }
    });
    expect(deterministicCleanupFailure).toMatchObject({ status: "cleanup", evaluationMode: "deterministic-only", lifecycle: { judge: "not-applicable", report: "not-written", cleanup: "failed" }, files: { metadata: "metadata.json" } });
  ...
  it("creates atomic marked latest envelopes from attempted runs only and preserves prior latest on failure", async () => {
    ...
    const latest = createAuthoredWorkbookEvalLatestEnvelope({ generatedAt: "2026-08-29T00:00:00.000Z", invocation: { scope: "scenario", scenarioIds: [authored.id], repeat: 2 }, runs: [success, interrupted] });
    ...
    expect(latest.runs[0]).toMatchObject({ ...AUTHORED_WORKBOOK_EVAL_MARKERS, scenario: authored.id, files: AUTHORED_WORKBOOK_REPORT_FILENAMES });
    ...
    const missing = createAuthoredWorkbookEvalLatestEnvelope({ invocation: { scope: "scenario", scenarioIds: [authored.id], repeat: 1 }, runs: [createAuthoredWorkbookEvalLatestRunEntry({ ...success, reportDirectory: "missing-run" })] });
    ...
    expect(() => createAuthoredWorkbookEvalLatestEnvelope({ invocation: { scope: "scenario", scenarioIds: [authored.id], repeat: 2 }, runs: [success, success] })).toThrow(/duplicate/i);
    expect(() => createAuthoredWorkbookEvalLatestEnvelope({ invocation: { scope: "scenario", scenarioIds: [authored.id], repeat: 1 }, runs: [createAuthoredWorkbookEvalLatestRunEntry({ ...success, reportDirectory: "/tmp/private" })] })).toThrow(/directory/i);
    ...
    expect(() => createAuthoredWorkbookEvalLatestEnvelope({ invocation: { scope: "release", scenarioIds: [authored.id], repeat: 2 }, runs: [] })).toThrow(/release/i);
  });

  it("rejects adversarial latest curated content changes without replacing prior latest", async () => {
    const root = await mkdtemp(join(tmpdir(), "authored-latest-curated-adversarial-"));
    tempRoots.push(root);
    const { latest, latestText: prior } = await authoredJudgedLatestFixture(root, "run-001");
    const judgeInputPath = join(root, "run-001", "judge-input.json");
    const summaryPath = join(root, "run-001", "summary.md");
    const originalJudgeInput = await readJson<Record<string, unknown>>(judgeInputPath);
    const originalSummary = await readFile(summaryPath, "utf8");
    const writeJson = (path: string, value: unknown) => atomicWriteText(path, `${JSON.stringify(value, null, 2)}\n`);
    const expectLatestRejectedAndUnchanged = async (mutate: () => Promise<void>): Promise<void> => {
      try {
        await mutate();
        await expect(writeAuthoredWorkbookEvalLatestEnvelope(root, latest)).rejects.toThrow("Unable to update authored workbook latest report.");
        await expect(readFile(join(root, "latest.json"), "utf8")).resolves.toBe(prior);
      } finally {
        await writeJson(judgeInputPath, originalJudgeInput);
        await atomicWriteText(summaryPath, originalSummary);
      }
    };
  ...
  it("rejects adversarial latest metadata identity changes without replacing prior latest", async () => {
    ...
    tempRoots.push(root);
    const { scenario: authored, latest, latestText: prior } = await authoredJudgedLatestFixture(root, "run-001");
    const success = latest.runs[0]!;
    ...
    await expectLatestRejectedAndUnchanged(async () => writeMetadata(createAuthoredWorkbookEvalFailureMetadataEnvelope({ runId: "run-001", scenarioId: authored.id, status: "gate", modelIdentities: modelIdentities() })));
    ...
    const differingButValidLatest = createAuthoredWorkbookEvalLatestEnvelope({
      generatedAt: "2026-08-29T00:00:00.000Z",
      invocation: { scope: "scenario", scenarioIds: [authored.id], repeat: 1 },
    ...
    expect(() => createAuthoredWorkbookEvalLatestEnvelope({
      invocation: { scope: "scenario", scenarioIds: [authored.id, "other-public-scenario"], repeat: 1 },
      runs: [
        success,
