...
class Lessons003004WorkflowFakeTutor extends RecordingMainTutor {
  ...
  protected override async decide(input: ReviewInput): Promise<TutorDecision> {
    ...
    const terminalReviewsForBlock = this.reviews.filter((review) => review.attempt.evidence.kind === "terminal" && review.attempt.blockId === "lesson--003-build-a-validator--implementation-order");
    if (input.attempt.evidence.kind === "terminal" && input.attempt.blockId === "lesson--003-build-a-validator--implementation-order" && terminalReviewsForBlock.length === 1) {
...
describe("authored workbook scenario descriptors", () => {
  it("declares the four release journeys with budget source-of-truth model-call counts", () => {
    ...
    for (const scenario of AUTHORED_WORKBOOK_SCENARIOS) {
      ...
      expect(Object.keys(scenario.expectedModelCalls).sort()).toEqual(["judge", "mainTutor", "total"]);
      ...
      if (scenario.id === "lesson-001-headless-boundary") expect(scenario.expectedModelCalls.mainTutor).toBeGreaterThan(13);
      if (scenario.id === "lessons-003-004-evidence-feedback") expect(scenario.expectedModelCalls.mainTutor).toBeGreaterThan(28);
      if (scenario.id === "lesson-013-operator-judgement") expect(scenario.expectedModelCalls.mainTutor).toBeGreaterThan(14);
    }
  ...
  it("repairs legitimate Lesson 013 reflection feedback before completing the checkpoint", async () => {
    ...
    expect(scenario.expectedModelCalls.mainTutor).toBe(16);
  ...
  it("captures runner-private gate checkpoints in authored order with an optional no-op default", async () => {
    ...
    const checkpointRecorder = createAuthoredWorkbookScenarioGateCheckpointRecorder(authoredWorkbookScenarioById("primer-validation-misconception"));
