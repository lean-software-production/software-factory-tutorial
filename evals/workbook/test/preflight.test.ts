...
async function expectPreflightFailure(input: AuthoredWorkbookEvalPreflightRequestInput, operations: Partial<AuthoredWorkbookEvalExternalOperations>, signal?: AbortSignal): Promise<AuthoredWorkbookEvalPreflightError> {
  try {
    await runAuthoredWorkbookEvalPreflight(input, { operations, signal, timeoutsMs: { npmVersion: 5, dockerReady: 5, terminalStart: 5, terminalReadiness: 5, terminalAuth: 5, terminalCleanup: 5, fixture: 5, fixtureCleanup: 5, mainTutor: 5, judge: 5 } });
  } catch (error) {
...
async function pauseableFixtureTimeoutCase(pauseAfter: "root" | "calculator" | "stub"): Promise<{ error: AuthoredWorkbookEvalPreflightError; events: string[]; root: string; closed: number }> {
  ...
  try {
    const failure = (async () => {
      try {
        await runAuthoredWorkbookEvalPreflight(validRequest(), { operations, timeoutsMs: { npmVersion: 5, dockerReady: 5, terminalStart: 5, terminalReadiness: 5, terminalAuth: 5, terminalCleanup: 5, fixture: 100, fixtureCleanup: 5, mainTutor: 5, judge: 5 } });
      } catch (error) {
        expect(error).toBeInstanceOf(AuthoredWorkbookEvalPreflightError);
        return error as AuthoredWorkbookEvalPreflightError;
      }
...
describe("authored workbook eval preflight", () => {
  ...
  it("validates repeat defaults and multiplies only selected release scenario calls", async () => {
    ...
    expect(defaultRepeat.scenarios[0]?.expectedModelCalls).toEqual({ mainTutor: 14, judge: 1, total: 15 });
    expect(defaultRepeat.expectedCosts.paidReleaseCallsByRole).toEqual({ "Main Tutor": 14, Judge: 1 });
    expect(defaultRepeat.expectedCosts.paidPreflightCallsByRole).toEqual({ "Main Tutor": 1, Judge: 1 });
    ...
    expect(request.expectedCosts.paidPreflightCallsByRole).toEqual({ "Main Tutor": 1, Judge: 1 });
    expect(request.expectedCosts.expectedPaidPreflightCalls).toBe(2);
    expect(request.expectedCosts.paidReleaseCallsByRole).toEqual({ "Main Tutor": 42, Judge: 3 });
    expect(request.expectedCosts.expectedPaidReleaseCalls).toBe(45);
    expect(request.expectedCosts.expectedPaidModelCallsTotal).toBe(47);
    ...
    expect(summary.counts.paidPreflightCallsByRole).toEqual({ "Main Tutor": 1, Judge: 1 });
    expect(summary.counts.paidReleaseCallsByRole).toEqual({ "Main Tutor": 42, Judge: 3 });
  ...
  it("rejects invalid, missing, or duplicate repeat flags and repeat underbudgets before side effects", async () => {
    ...
    expect(() => parseAuthoredWorkbookEvalPreflightArgs([
      "--scenario", "part-1-happy-path",
      "--max-paid-model-calls", "13",
      "--max-estimated-tokens", "26000",
      "--repeat", "3"
    ...
    const error = await expectPreflightFailure(validRequest({
      repeat: 3,
      costBudget: { maxPaidModelCalls: 46, maxEstimatedTokens: 96_000, estimatedTokensPerPaidCall: 2_000 }
  ...
  it("fails pure argument and budget validation before npm, Docker, stubs, artifacts, or paid calls", async () => {
    ...
    const missingOnePreflightProbe = await expectPreflightFailure(validRequest({ costBudget: { maxPaidModelCalls: 16, maxEstimatedTokens: 36_000, estimatedTokensPerPaidCall: 2_000 } }), recordingOperations(events));
  ...
  it("runs every unpaid check before paid Main Tutor then actual Judge command", async () => {
    const events: string[] = [];
    const summary = await runAuthoredWorkbookEvalPreflight(validRequest(), {
      operations: recordingOperations(events),
      timeoutsMs: { npmVersion: 5, dockerReady: 11, fixture: 5, fixtureCleanup: 5, mainTutor: 6, judge: 8 }
    });
  ...
  it("bounds every timeout stage and reports the correct sanitized phase", async () => {
    ...
    for (const [stage, phase] of expectations) {
      ...
      if (phase !== "mainTutor" && phase !== "judge") expect(events.some((event) => event.startsWith("paid:"))).toBe(false);
    }
  });
  ...
  it("produces a strictly public summary with budget warning, release budget, and same env identities", async () => {
    ...
    expect(summary.counts.paidReleaseCallsByRole).toEqual({ "Main Tutor": 30, Judge: 2 });
    expect(summary.counts.expectedPaidPreflightCalls).toBe(2);
    expect(summary.counts.expectedPaidReleaseCalls).toBe(32);
    expect(summary.counts.expectedPaidModelCallsTotal).toBe(34);
    expect(summary.counts.expectedEstimatedTokensTotal).toBe(68_000);
    expect(summary.expectedBudgetFlags).toEqual({});
    expect(summary.expectedCapabilityFlags).toEqual({});
    expect(summary.warnings.join("\n")).toContain("Main Tutor and Judge");
  ...
  it("snapshots only needed config keys and ignores later global env changes", () => {
    const env = { PATH: "p", HOME: "h", OPENCODE_API_KEY: secret, EVAL_JUDGE_COMMAND: "judge", TUTOR_MODEL: "anthropic/a", EVAL_JUDGE_MODEL: "google/c", EXTRA_SECRET: "no" };
