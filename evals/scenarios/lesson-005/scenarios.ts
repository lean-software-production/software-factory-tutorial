...
export const lesson005Scenarios: Scenario[] = [
  ...
  { id: "mistake-criteria-not-handed-to-the-doer", lesson: "005", mode: "mistake", description: "Hands-on learner writes the criteria but leaves the run script's doer turn sending only `refactor.md`.", expectedMistake: "The doer's prompt defers to criteria nobody hands it, so the station is asked to work towards something it never receives.", seed: partOneSeed, patches: [relocateStep(), successStep(), criteriaStep(), unsharedDefect, runRepair(unsharedDefect)], finalState: lesson005FinalState },
  { id: "mistake-line-without-enter-pause", lesson: "005", mode: "mistake", description: "Hands-on learner writes the loop without the Enter pause between iterations.", expectedMistake: "The line never hands control back, so it keeps spending station turns on the calculator with nobody reading the verdicts.", seed: partOneSeed, patches: [relocateStep(), successStep(), criteriaStep(), noPauseDefect, runRepair(noPauseDefect)], finalState: lesson005FinalState }
];
