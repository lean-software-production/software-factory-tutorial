...
import {
  assertV2EvalPrivatePromptLoggingDisabled,
  ...
  return {
    ...
    preflightWorkbookModels: async () => {
      ...
      return [
        ...
        { role: "Practice Coach", envVar: "PRACTICE_COACH_MODEL", requested: "provider/coach", selectedModel: { provider: "provider", id: "coach" } }
      ];
    },
...
const controlledEvalEnv = ["EVAL_JUDGE_MODEL", "OPENCODE_API_KEY", "PRACTICE_COACH_LOG_PROMPT", "TUTOR_MODEL", "PRACTICE_COACH_MODEL", "EVAL_JUDGE_COMMAND"] as const;
...
describe("v2 live eval preflight", () => {
  it("rejects forbidden private prompt logging before disposable fixtures or probes", async () => {
    expect(() => assertV2EvalPrivatePromptLoggingDisabled({ PRACTICE_COACH_LOG_PROMPT: "1" })).toThrow("private prompt logging is forbidden");
    const calls: string[] = [];
    const reportsRoot = await tempChild("reports");

    await withEvalEnvironment({ EVAL_JUDGE_MODEL: "provider/judge", PRACTICE_COACH_LOG_PROMPT: "1" }, async () => {
      await expect(runV2EvalCli(["--scenario", "v2-exact-command-success"], {
        reportsRoot,
        engineRoot,
        preflightDependencies: orderedDeps(calls)
      })).rejects.toThrow("private prompt logging is forbidden");
    });
  ...
  it("checks explicit judge model before private logging and other probes", async () => {
    const calls: string[] = [];
    const reportsRoot = await tempChild("reports");

    await withEvalEnvironment({ PRACTICE_COACH_LOG_PROMPT: "1" }, async () => {
      await expect(runV2EvalCli(["--scenario", "v2-exact-command-success"], {
        reportsRoot,
        engineRoot,
        preflightDependencies: orderedDeps(calls)
  ...
  it("runs live preflight probes in deterministic fail-fast order before report directories", async () => {
    for (const [stage, expected, message] of [
      ...
      ["models", ["fixture", "docker", "models"], "checking Tutor and Practice Coach model connectivity"],
