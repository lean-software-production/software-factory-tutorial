...
describe("workbook CLI", () => {
  ...
  it.each(["001", "001-run-an-agent-headlessly", "007", "007-compose-and-branch"])("starts --lesson %s from the normal materialized workspace at its target", async (lesson) => {
    ...
    const startServer = vi.fn(async (options: WorkbookServerOptions) => await startWorkbookServer({ ...options, webRoot: resolve(contentRoot, "web"), embeddedTerminal: false, mainTutor, practiceCoach: { assess: async () => ({ outcome: "ready" as const, text: "" }), dispose() {} } }));
