...
describe("TerminalEvidenceRepository", () => {
  it("writes immutable running and finished snapshots outside the timeline", async () => {
    const repository = new TerminalEvidenceRepository({ stateRoot: await stateRoot() });
    const runningRef = await repository.writeRunning({
      command: "npm test",
      interactions: [{ kind: "input", data: "npm test\r" }, { kind: "output", data: "PASS\n" }],
    });
    const finishedRef = await repository.writeFinished({
      command: "npm test",
      interactions: [{ kind: "output", data: "PASS\n" }],
      exitStatus: 0,
    });

    expect(runningRef).toMatch(/^[0-9a-f-]{36}$/i);
    expect(finishedRef).not.toBe(runningRef);
    await expect(repository.read(runningRef)).resolves.toEqual({
      kind: "running",
      command: "npm test",
      interactions: [{ kind: "input", data: "npm test\r" }, { kind: "output", data: "PASS\n" }],
    });
  ...
  it("refuses a reference collision instead of overwriting the original snapshot", async () => {
    ...
    await repository.writeRunning({ command: "first", interactions: [] });

    await expect(repository.writeRunning({ command: "second", interactions: [] })).rejects.toMatchObject({ code: "EEXIST" });
    await expect(repository.read(evidenceRef)).resolves.toEqual({ kind: "running", command: "first", interactions: [] });
  });
  ...
  it("validates bounded snapshots when writing and when reading persisted JSON", async () => {
    ...
    await expect(repository.writeRunning({ command: "x".repeat(MAX_TERMINAL_COMMAND_BYTES + 1), interactions: [] }))
    ...
    await writeFile(path, "{\"kind\":\"running\",\"command\":\"npm test\",\"interactions\":[{\"kind\":\"output\",\"data\":42}]}\n", "utf8");
