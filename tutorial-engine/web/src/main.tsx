...
import { resolvedChoiceSelections } from "./choice-state.js";
...
function TranscriptEvent({ event, send, disabled, selectedOptionId }: { event: Event; send: (message: BrowserMessage) => void; disabled: boolean; selectedOptionId?: string }) {
  switch (event.type) {
    ...
    case "choice": return <ChoiceCard event={event} send={send} disabled={disabled} selectedOptionId={selectedOptionId} />;
...
function App() {
  ...
  const resolvedChoices = useMemo(() => resolvedChoiceSelections(events), [events]);
  ...
  return <main><header><div><p className="eyebrow">LOCAL TUTORIAL</p><h1>{title}</h1><nav className="progress" aria-label="Tutorial progress">{progress.map((item, index) => <Fragment key={item.id}>{item.part && item.part !== progress[index - 1]?.part ? <span className="part">{item.part}</span> : null}<span className={item.state}>{item.label}</span></Fragment>)}</nav></div><span className={`state ${serverConnection === "connected" ? state : serverConnection}`}>{serverConnection === "connected" ? state.replace("-", " ") : serverConnection === "connecting" ? "connecting" : "server stopped"}</span></header>
    <section className="transcript" aria-live="polite">{serverConnection === "disconnected" && <Card title="Tutorial server stopped" className="error server-stopped"><p>The tutorial server is no longer available. Restart it and reopen its URL to continue.</p></Card>}<SessionStartCard session={session} send={send} disabled={!serverConnected} />{events.map((event, index) => <TranscriptEvent key={`${event.type}-${index}`} event={event} send={send} disabled={!serverConnected} selectedOptionId={event.type === "choice" ? resolvedChoices.get(event.id) : undefined} />)}{serverConnected && session.state !== "select" && state === "working" && <div className="thinking" role="status"><span className="spinner" aria-hidden="true" /><span aria-hidden="true">{activityCaption(activity)}</span><span className="visually-hidden">Working…</span></div>}</section>
