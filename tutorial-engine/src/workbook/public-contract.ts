...
export interface PublicWorkbookState { workbook: { title: string }; introduction: string; introductionComplete: boolean; chapters: PublicWorkbookChapter[]; progress: PublicWorkbookProgress; adapter: { note?: string; modelBackedHelp?: boolean; /** A persisted --lesson session; only exact `move on` may skip evaluated work. */ testOnlyJump?: boolean }; orderedBlocks?: PublicWorkbookOrderedBlock[]; revealedBlockIds?: string[]; renderedBlockIds?: string[]; readyBlockIds?: string[]; currentBlock?: PublicWorkbookOrderedBlock & { workAccepted?: boolean }; completion?: { complete: true; anchorId: string; summary?: string }; timeline: readonly PublicTimelineRecord[]; }
...
export function isPublicWorkbookState(value: unknown): value is PublicWorkbookState {
  ...
  if (value.adapter.testOnlyJump !== undefined && typeof value.adapter.testOnlyJump !== "boolean") return false;
