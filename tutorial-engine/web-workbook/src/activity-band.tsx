 ...
 */
export function ActivityBand({ lessonId, activeBlock, progress, refresh, onRetry = async () => undefined, onTerminalInsertionChange, onEditorLocalRevision }: {
  lessonId: string;
  activeBlock: Block;
  progress: Progress;
  ...
  return <>
    <section ref={bandRef} className="current-activity-band" data-activity-type={activeBlock.type} data-activity-layout="scroll-linked" data-activity-preloaded={readyTerminalPreload ? "true" : undefined} aria-label="Activity">
      <BlockView lessonId={lessonId} block={activeBlock} progress={progress} refresh={refresh} onRetry={onRetry} onTerminalInsertionChange={forwardTerminalInsertion} onEditorLocalRevision={onEditorLocalRevision} />
    </section>
  </>;
}
